...
def loaddata(self):
    ...
    self.lineEdit_2.setText(gest.nom_complet)
...
def validate(self):
    try:
        ...
        if name == "":
            ...
            # start the app
            retval = msg.exec_()
        elif name == gest.nom_complet:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)

            # setting message for Message Box
            msg.setText(
                "Vous n'avez effectuer aucun changement. Veuillez changer le nom ou cliquer sur le boutton retour.")
            ...
            request = f"UPDATE `gestionstock`.`gestionnaire` SET `nomComplet` = '{name}' WHERE (`idgestionnaire` = '{Ui_Form.gestid}')"
...
class Ui_Form(object):
    ...
    def setupUi(self, Form):
        ...
        self.nmtextfield = QtWidgets.QLabel(Form)
        self.nmtextfield.setGeometry(QtCore.QRect(0, 50, 751, 451))
        self.nmtextfield.setStyleSheet("background-color:rgb(231, 231, 231)")
        self.nmtextfield.setText("")
        self.nmtextfield.setObjectName("nmtextfield")
        ...
        self.lineEdit_2.setGeometry(QtCore.QRect(370, 160, 241, 31))
        font = QtGui.QFont()
        font.setPointSize(13)
        ...
        self.pushButton_2.setGeometry(QtCore.QRect(630, 160, 26, 26))
